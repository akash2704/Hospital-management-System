...
def medical_tests():
    st.header('COVID TEST')
    option_list = ['', 'Add test', 'Update test', 'Delete test', 'Show test of a particular patient']
    option = st.sidebar.selectbox('Select function', option_list)
    t = Medical_Test()
    if (option == option_list[1] or option == option_list[2] or option == option_list[3]) and verify_dr_mls_access_code():
        if option == option_list[1]:
...
def home():
    ...
    option = st.sidebar.selectbox('Select module', ['', 'Covid Patients', 'Doctors', 'Prescriptions', 'Covid Tests', 'Departments'])
    if option == 'Covid Patients':
        patients()
    elif option == 'Doctors':
        doctors()
    elif option == 'Prescriptions':
        prescriptions()
    elif option == 'Covid Tests':
        medical_tests()
